...
<figure>
  <img src="https://cdn-images-1.medium.com/max/800/0*Vt_QSAYZSpJT2_Mt" />
  <figcaption>
    _Photo by [Franck -
    Free-Hotspot.com](https://unsplash.com/@franckinjapan?utm_source=medium&utm_medium=referral)
...
This turns your _unencrypted_ `AsyncStorage` into a safe vault that can only be unlocked by the person owning that key. In most cases and unless someone got their phone stolen and compromised that’s the primary user of the device.
...
The fact that the secure random key has a high degree of randomness, is not available at build time and can’t be easily guessed or recreated makes the encrypted data in your `AsyncStorage` _much_ safer, and completely useless to anyone without it.
...
<figure>
  ...
  />
  <figcaption>
    _A high-level overview of the components involved in this **secure storage**
    mechanism (<a href={dataFlow.src}>big version</a>)_
  </figcaption>
</figure>
...
<figure>
  ...
  />
  <figcaption>
    _Bird’s eye view of the **EncryptionGate** and **getEncryptionKey** method (
    <a href={dataFlowEncryptionGate.src}>big version</a>)_
  </figcaption>
</figure>
...
- `binaryToBase64`: converts these bytes to a base64 string. This is our new, [cryptographically-secure](https://github.com/rh389/react-native-securerandom#react-native-securerandom) password. 🎊
...
If the `key` turns out to be fresh _and_ there’s already data in our store, we have a problem: the data in our store can not be decrypted anymore, since a newly generated key won’t match the previous one.
...
It’s very similar to what a `configureStore()` call does in many applications, only that it accepts and returns some additional parameters: `encryptionKey`, `hasData` go _in_. And an object with two props (`store`, `persistor`) comes _out_.
...
<figure>
  ...
  />
  <figcaption>
    _The **Store Gate** and **generateStore** function at a glance (
    <a href={dataFlowStoreGate.src}>big version</a>)_
  </figcaption>
</figure>
...
- [redux-persist-encrypted-async-storage](https://github.com/chaudhryjunaid/redux-persist-encrypted-async-storage/blob/master/index.js): a simple encrypted wrapper around `AsyncStorage` without any Redux dependencies. Be aware though that it’s using `uuidv4` which is slightly less secure than the library we’re using above, `react-native-securerandom`.
...
_Originally published in [The Startup](https://medium.com/swlh/a-bullet-proof-approach-to-storing-sensitive-user-data-in-react-native-ab3f7a2779f9) on December 22, 2020._
